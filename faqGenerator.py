...
def chat(prompt, questions=""):
    ...
    if questions != "":
        message_history.append(
            {"role": "assistant", "content": "Generated FAQ and their answers are :\n"}
        )
    ...
    response = client.chat.completions.create(
        model="gpt-4o-mini", messages=message_history
    )
    return response
...
def generate_faq_multithreaded(md_files):
    ...
    index = 1
    number_of_questions = 5
    print("generate_faq_multithreaded")
    print(md_files)
    # with ThreadPoolExecutor() as executor:
    for file in md_files:
        print("file", file)
        result = process_file(file, index)
        print("result", result)
...
def string_to_list(faq, list):
    ...
    for index in range(len(faq_items)):
        ...
        if (not (item.isspace() or item == "")) and re.match(r"^Q\d", item):
            ...
            while index < len(faq_items) - 1 and not re.match(
                r"^Q\d", faq_items[index + 1]
            ):
