...
class FAQSectionService:
    ...
    async def insert_faq_section(self, user_email: str, faq_section: FAQSection) -> FAQSectionDB:
        ...
        inserted_faq_section = await self.faq_repo.insert_faq_section(faq_section_db, faq_items_db)

        return inserted_faq_section
    
    async def get_faq_sections(self, user_guid: UUID) -> list[FAQSectionDB]:

        query = select(FAQSectionDB, UserDB).where(UserDB.guid == user_guid, FAQSectionDB.user_id == UserDB.id)
        print(query)
    ...
    async def update_faq_section_visibility(self, section_guid: UUID, visibility: bool) -> FAQSectionDB:
        
        section = await self.faq_repo.update_faq_section_visibility(section_guid, visibility)
        return section
