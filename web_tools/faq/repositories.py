...
class UserRepository:
    ...
    async def get_user_by_email(self, email: str) -> UserDB:
        try:
            user_query = await self.session.execute(
                select(UserDB).filter(UserDB.email == email)
            )
    ...
    async def get_user_by_name(self, name: str) -> UserDB:
        try:
            user_query = await self.session.execute(
                select(UserDB).filter(UserDB.username == name)
            )
            user = user_query.scalar_one()
            return user
        except NoResultFound:
...
class FAQRepository:
    ...
    async def get_faq_section(self, section_guid: UUID) -> FAQSectionDB:
        query = (
            select(FAQSectionDB)
            .options(selectinload(FAQSectionDB.items))
            .where(FAQSectionDB.guid == section_guid)
        )
    ...
    async def get_faq_section_by_id(self, section_id: int) -> Optional[FAQSectionDB]:
        result = await self.session.execute(
            select(FAQSectionDB).where(FAQSectionDB.id == section_id)
        )
        return result.scalar_one_or_none()

    async def get_faq_section_by_guid(
        self, section_guid: str
    ) -> Optional[FAQSectionDB]:
        result = await self.session.execute(
            select(FAQSectionDB).where(FAQSectionDB.guid == section_guid)
        )
        return result.scalar_one_or_none()

    async def insert_faq_section(
        self, faq_section: FAQSectionDB, faq_items: List[FAQItemDB]
    ) -> FAQSectionDB:
    ...
    async def append_faq_item(
        self, section_id: int, faq_item: FAQItemDB
    ) -> FAQSectionDB:
    ...
    async def update_faq_section_visibility(
        self, section_guid: UUID, visbility: bool
    ) -> FAQSectionDB:
...
async def insert_faq_section(
    ...
    faq_section_db = FAQSectionDB(
        title=faq_section.title, description=faq_section.description, user_id=user.id
    )
    ...
    inserted_faq_section = await faq_repo.insert_faq_section(
