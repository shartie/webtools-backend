...
class UserDB(Base):
    ...
    accounts = relationship(
        "AccountDB", back_populates="user", cascade="all, delete-orphan"
    )
...
class AccountDB(Base):
    ...
    userId = Column(
        String,
        ForeignKey(f"{settings.database.schema}.User.id", ondelete="CASCADE"),
        nullable=False,
    )
...
class FAQSectionDB(Base):
    ...
    user_id = Column(
        String,
        ForeignKey(f"{settings.database.schema}.User.id", ondelete="SET NULL"),
        nullable=True,
    )
...
class FAQItemDB(Base):
    ...
    faq_section_id = Column(
        Integer,
        ForeignKey(f"{settings.database.schema}.faq_section.id", ondelete="CASCADE"),
    )
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), onupdate=func.now())
...
summary_authors = Table(
    ...
    Column(
        "summary_id",
        UUID(as_uuid=True),
        ForeignKey(f"{settings.database.schema}.summaries.id", ondelete="CASCADE"),
        primary_key=True,
    ),
    Column(
        "author_id",
        UUID(as_uuid=True),
        ForeignKey(f"{settings.database.schema}.authors.id", ondelete="CASCADE"),
        primary_key=True,
    ),
    schema=settings.database.schema,
)
...
class AuthorDB(Base):
    ...
    summaries = relationship(
        "SummaryDB", secondary=summary_authors, back_populates="authors"
    )
...
class SummaryDB(Base):
    ...
    authors = relationship(
        "AuthorDB", secondary=summary_authors, back_populates="summaries"
    )
    user = relationship("UserDB")  # Assuming a User model exists
    sources = relationship(
        "SourceDB", back_populates="summary", cascade="all, delete-orphan"
    )


# Async engine and session creation
# DATABASE_URL = os.environ.get("DATABASE_URL_ASYNCPG")
DATABASE_URL = get_settings().sqlalchemy_database_uri.render_as_string(
    hide_password=False
)
print(DATABASE_URL)
